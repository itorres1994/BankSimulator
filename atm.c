  ...
  switch(c) { 
   ...
   case BALANCE:
      ...
      if(check_pipe_write(status) != SUCCESS) {
         return check_pipe_write(status);
      }
      status = read(atm_in_fd,atmcmd.cmd, MESSAGE_SIZE);
      if(check_pipe_read(status) != SUCCESS) {
         return check_pipe_read(status);
      }
      ...
      switch(c) {
       ...
       default:
        ...
        error_msg(ERR_UNKNOWN_CMD, "Received an unknown command");
        break;
      } 
      //return check_pipe_write(status);
      break;
